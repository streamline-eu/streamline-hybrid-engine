...
/**
 * {@link SortedMapState} implementation that stores state in RocksDB.
 *
 ...
 */
public class RocksDBSortedMapState<K, N, UK, UV>
	extends AbstractRocksDBState<K, N, SortedMapState<UK, UV>, SortedMapStateDescriptor<UK, UV>, SortedMap<UK, UV>>
	...
	private final RocksDBNavigator navigator;

	/**
	 * Creates a new {@code RocksDBSortedMapState}.
	 *
	 * @param namespaceSerializer The serializer for the namespace.
	 * @param stateDesc The state identifier for the state.
	 */
	public RocksDBSortedMapState(ColumnFamilyHandle columnFamily,
		...
		navigator = new RocksDBNavigator();
	}

	// ------------------------------------------------------------------------
	//  SortedMapState Implementation
	// ------------------------------------------------------------------------
	...
	@Override
	public Navigator<UK, UV> navigator() throws Exception {
		final byte[] prefixBytes = serializeCurrentKeyAndNamespace();

		navigator.fillPrefixBytes(prefixBytes);
	...
	public byte[] getSerializedValue(byte[] serializedKeyAndNamespace) throws Exception {
		...
		//TODO make KvStateRequestSerializer key-group aware to save this round trip and currentKey-group computation
	...
	private class RocksDBMapEntry implements Map.Entry<UK, UV> {
		...
		/** The raw bytes of the key stored in RocksDB. Each user currentKey is stored in RocksDB
		...
		/** The user key and value. The deserialization is performed lazily, i.e. the currentKey
	...
	private abstract class RocksDBMapIterator<T> implements Iterator<T> {
		...
		/**
		 * The prefix bytes of the key being accessed. All entries under the same currentKey
	...
	private class RocksDBNavigator implements Navigator<UK, UV> {

		private RocksIterator iterator;
		private byte[] keyPrefixBytes;

		private byte[] cachedSeekKey;
		...
		private UK cachedUserKey;
		private UV cachedUserValue;

		public void fillPrefixBytes(final byte[] keyPrefixBytes) {
			this.keyPrefixBytes = keyPrefixBytes;
		...
		public void seek(UK userKey) throws Exception {
			...
			userKeySerializer.serialize(userKey, keySerializationDataOutputView);
			byte[] seekKey = cachedSeekKey;
			// reuse byte array
			if (seekKey != null && keySerializationStream.getPosition() == seekKey.length) {
				System.arraycopy(keySerializationStream.getBuf(), 0, seekKey, 0, seekKey.length);
			} else {
				cachedSeekKey = keySerializationStream.toByteArray();
		...
		@Override
		public UK key() throws Exception {
			if (currentKey != null) {
				if (cachedUserKey == null) {
					cachedUserKey = deserializeUserKey(currentKey);
				}
		...
		private void updateCurrentKey(final int direction) {
			...
			// and this is not a seek operation
			if (direction != 0) {
			...
			if (!iterator.isValid()) {
				...
				rawKeyBytes = iterator.key();
			}

			// check if we are still in key prefix
			// update current key
			if (rawKeyBytes != null && underSameKey(rawKeyBytes)) {
				currentKey = rawKeyBytes;

				// increase next context
				if (direction > 0) {
					nextContext++;
				}
				// increase prev context
				else if (direction < 0) {
					prevContext--;
				}
			} else {
