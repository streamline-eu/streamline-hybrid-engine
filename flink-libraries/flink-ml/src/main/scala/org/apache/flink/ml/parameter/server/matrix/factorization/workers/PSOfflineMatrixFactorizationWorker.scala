...
import org.apache.flink.ml.parameter.server.matrix.factorization.PSOfflineMatrixFactorization
import org.apache.flink.ml.parameter.server.matrix.factorization.factors.{FactorInitializer, RangedRandomFactorInitializerDescriptor, SGDUpdater}
import org.apache.flink.ml.parameter.server.matrix.factorization.utils.{EOF, Rating}
    ...
    value match {
      case Right(rating) =>
        ...
        for(_  <- 1 to Math.min(allItemIdsSet.size - seenSet.size, negativeSampleRate)) {
          ...
          rs += Rating.fromTuple(rating.user, randomItemId, 0)
        }
